...
public:
	...
	VF( SendDatagram, int, 46, bf_write* unk = NULL );
	VF( Transmit, bool, 49, bool onlyReliable );
};

class IVEngineClient
{
public:
	virtual int                   GetIntersectingSurfaces( const model_t *model, const Vec3D &vCenter, const float radius, const bool bOnlyVisibleSurfaces, SurfInfo *pInfos, const int nMaxInfos ) = 0;
	virtual Vec3D                GetLightForPoint( const Vec3D &pos, bool bClamp ) = 0;
	virtual IMaterial*            TraceLineMaterialAndLighting( const Vec3D &start, const Vec3D &end, Vec3D &diffuseLightColor, Vec3D& baseColor ) = 0;
	...
	virtual bool                  DoesBoxTouchAreaFrustum( const Vec3D &mins, const Vec3D &maxs, int iArea ) = 0; // 60
	...
	virtual void                  ComputeLighting( const Vec3D& pt, const Vec3D* pNormal, bool bClamp, Vec3D& color, Vec3D *pBoxColors = NULL ) = 0;
	virtual void                  ActivateOccluder( int nOccluderIndex, bool bActive ) = 0;
	virtual bool                  IsOccluded( const Vec3D &vecAbsMins, const Vec3D &vecAbsMaxs ) = 0; // 74
	...
	virtual int                   IsSkyboxVisibleFromPoint( const Vec3D &vecPoint ) = 0;
	...
	virtual void                  ComputeLightingCube( const Vec3D& pt, bool bClamp, Vec3D *pBoxColors ) = 0;
	...
	virtual void                  SolidMoved( class IClientEntity *pSolidEnt, class ICollideable *pSolidCollide, const Vec3D* pPrevAbsOrigin, bool accurateBboxTriggerChecks ) = 0;
	virtual void                  TriggerMoved( class IClientEntity *pTriggerEnt, bool accurateBboxTriggerChecks ) = 0;
	virtual void                  ComputeLeavesConnected( const Vec3D &vecOrigin, int nCount, const int *pLeafIndices, bool *pIsConnected ) = 0;
	...
	virtual void                  SpherePaintSurface( const model_t* model, const Vec3D& location, unsigned char chr, float fl1, float fl2 ) = 0;
	virtual bool                  HasPaintmap( void ) = 0;
	virtual void                  EnablePaintmapRender( ) = 0;
	//virtual void                TracePaintSurface( const model_t *model, const Vec3D& position, float radius, CUtlVec3D<Color>& surfColors ) = 0;
	virtual void                  SphereTracePaintSurface( const model_t* model, const Vec3D& position, const Vec3D &vec2, float radius, /*CUtlVec3D<unsigned char, CUtlMemory<unsigned char, int>>*/ int& utilVecShit ) = 0;
	...
	VF( GetNetChannel, NetChannel*, 78 );
};